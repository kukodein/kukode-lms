...
class PayoutsController extends Controller
{
    ...
    {
        $user = apiAuth();
        $payouts = Payout::where('user_id', $user->id)->where('status',Payout::$done)
            ->orderBy('status', 'asc')
    ...
    public function requestPayout()
    {
        ...
        if ($getUserPayout < $getFinancialSettings['minimum_payout']) {
            return apiResponse2(0, 'minimum_payout', trans('api.financial.minimum_payout'));
        }
        ...
        return apiResponse2(0, 'identity_settings', trans('api.financial.identity_settings'));
