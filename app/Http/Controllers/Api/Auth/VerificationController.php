...
class VerificationController extends Controller
{
    ...
    public function checkConfirmed2($user = null, $username, $value)
    {
        if (!empty($value)) {
            $verification = Verification::where($username, $value)
                //->where('expired_at', '>', time())
    ...
    public function sendMsg($user = null, $username, $value)
    {
        if (!empty($value)) {
            $verification = Verification::where($username, $value)
                ->where('expired_at', '>', time())
    ...
    public function confirmCode(Request $request, $username=null)
    {
        ...
        if (empty($authUser)) {
            ...
            if (!empty($authUser)) {
                ...
                if (1 /*auth()->loginUsingId($authUser->id)*/) {

                    if (!empty($referralCode)) {
                        //   Affiliate::storeReferral($authUser, $referralCode);
                    }

                    return apiResponse2(1, 'verified', trans('auth.verified'));
